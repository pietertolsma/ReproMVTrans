...
class DepthOutput:
    ...
    def get_visualization_img(self, left_img_np, corner_scale=1, raw_disp=True):
        ...
        ] = disp_map_visualize(disp_scaled)
        return left_img_np

    def compute_loss(self, depth_targets, log=None, name=None, device="cpu"):
        if self.is_numpy:
            raise ValueError("Output is not in torch mode")
        depth_target_stacked = []
        for depth_target in depth_targets:
            depth_target_stacked.append(depth_target)
        depth_target_batch = torch.stack(depth_target_stacked)
        depth_target_batch = depth_target_batch.to(device)
