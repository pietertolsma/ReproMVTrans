...
def set_id_grid(h, w):
    i_range = Variable(torch.arange(0, h).view(1, h, 1).expand(1, h, w)).to(
        torch.float32
    )  # [1, H, W]
    j_range = Variable(torch.arange(0, w).view(1, 1, w).expand(1, h, w)).to(
        torch.float32
    )  # [1, H, W]
...
def cam2cam(cam_1_coords, extrinsic):
    ...
    :param extrinsic: [batch, 4, 4]  extrinsic (the position of the origin of the world
    coordinate system expressed
    in coordinates of the camera-centered coordinate system. world coords w.r.t. camera
    coords)
...
def pixel2cam(depth, intrinsics, pixel_coords, is_homogeneous=True):
    ...
    Camera model: [X,Y,Z]^T = D * K^-1 * [u,v,1]^T
    Args:
    ...
    cam_coords = intrinsics_inv.bmm(current_pixel_coords).view(
        b, 3, h, w
...
def normalize_pixel_coords(
    pixel_coords, padding_mode="zeros", image_height=None, image_width=None
):
    """
...
def normalize_pixel_coords_volume(
    pixel_coords,
    depth_min,
    depth_interval,
    padding_mode="zeros",
    ...
    if disp_min is None:
        Z_norm = 2 * ((Z - depth_min) / depth_interval) / (ndepths - 1) - 1.0
    else:
        # the cost volume use disparity planes not depth planes
        Z_norm = (
            2 * ((1.0 / (Z + 1e-10) - disp_min) / disp_interval) / (ndepths - 1) - 1.0
        )
    if padding_mode == "zeros":
        X_mask = ((X_norm > 1) + (X_norm < -1)).detach()
        X_norm[
        ...
        valid_mask = (~invalid_mask).view(
            batch_size, 1, ndepths, image_height, image_width
        )

    pixel_coords_norm = torch.stack(
        [X_norm, Y_norm, Z_norm], dim=3
    )  # [B, ndepths, H*W, 3]
    pixel_coords_norm = pixel_coords_norm.view(
        batch_size, ndepths, image_height, image_width, 3
    )
    return pixel_coords_norm, valid_mask
...
def inverse_warp(feat, depth, pose, intrinsics, pixel_coords, padding_mode="zeros"):
    ...
    Args:
        ...
        pixel_coords:  [b, 3, h, w] original homogeneous pixel coordinates of
        target image
    Returns:
    ...
    tgt_cam_coords = pixel2cam(
        depth, intrinsics, pixel_coords, is_homogeneous=True
    )  # [b, 4, h, w]

    src_cam_coords = cam2cam(
        tgt_cam_coords, torch.inverse(pose)
    )  # change pose to extrinsic matrix (world 2 camera)

    src_pixel_coords = cam2pixel(src_cam_coords, intrinsics)

    src_pixel_coords_norm, valid_mask = normalize_pixel_coords(
        src_pixel_coords
    )  # [B,H,W,2]

    projected_feat = torch.nn.functional.grid_sample(
        feat, src_pixel_coords_norm, padding_mode=padding_mode
    )
...
def warp_volume(
    ...
    tgt_cam_coords = pixel2cam(
        depth, cam_intr, pixel_coords, is_homogeneous=True
    )  # [b, 4, ndepths, h*w]
    ...
    src_pixel_coords_norm, valid_mask = normalize_pixel_coords_volume(
        src_pixel_coords,
        depth_min,
        depth_interval,
        image_height=H,
    ...
    if padding_mode == "border":
        ...
        projected_volume = torch.nn.functional.grid_sample(
            feat_volume_th_,
            src_pixel_coords_norm,
            inter_mode,
            padding_mode=padding_mode,
        )
    else:
        projected_volume = torch.nn.functional.grid_sample(
            feat_volume, src_pixel_coords_norm, inter_mode, padding_mode=padding_mode
        )
...
def warp_depth(depth, pose, cam_intr, pixel_coords):
    ...
    :param pose: [N,4,4] relative camera pose (Src_pose @ inv(ref_pose)),
    not camera extrinsic
    :param cam_intr: [N,3,3]
...
def skew(phi):
    ...
    Phi = torch.cat(
        [zero, -phi_2, phi_1, phi_2, zero, -phi_0, -phi_1, phi_0, zero], dim=-1
    )
    Phi = Phi.view(b, 3, 3)
    return Phi
...
def expMap(ksai):
    ...
    identities = (
        torch.eye(3).view(1, 3, 3).repeat(b, 1, 1).to(ksai.dtype).to(ksai.device)
    )  # [b, 3, 3]
...
def logMap(SE3):
    ...
    Omega = skew(omega)
    identities = (
        torch.eye(3).view(1, 3, 3).repeat(b, 1, 1).to(SE3.dtype).to(SE3.device)
    )  # [b, 3, 3]
    V_inv = (
        identities
        - 0.5 * Omega
        + (1 - theta / (2 * torch.tan(theta / 2)))
        * torch.bmm(Omega, Omega)
        / (theta * theta)
    )
...
def homo_warping(src_fea, src_proj, ref_proj, depth_values):
    ...
    warped_src_fea = torch.nn.functional.grid_sample(
        src_fea,
