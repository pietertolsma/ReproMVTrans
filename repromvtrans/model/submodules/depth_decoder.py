...
from repromvtrans.utils.epipolar_ops import set_id_grid, warp_volume
from repromvtrans.model.transformer.epipolar_transformer import EpipolarTransformer
from repromvtrans.utils.layer_operations import (
    convbn,
    convbnrelu_3d,
    convbntanh_3d,
)
...
class DepthHybridDecoder(nn.Module):
    def __init__(
        ...
        self.dispconv_0 = nn.Conv2d(
            self.num_ch_dec[0], self.num_output_channels, 3, 1, 1, 1, bias=True
        )
        ...
        self.dres2 = nn.Sequential(
            convbnrelu_3d(base_channels + 1, base_channels + 1, 3, 1, 1)
        )
        self.key_layer = nn.Sequential(
            convbnrelu_3d(base_channels + 1, base_channels // 2, 3, 1, 1)
        )
        self.value_layer = nn.Sequential(
            convbntanh_3d(base_channels + 1, base_channels // 2, 3, 1, 1)
        )
        self.stereo_head0 = nn.Sequential(
            convbnrelu_3d(base_channels // 2, base_channels // 2, 3, 1, 1),
            nn.Conv3d(
                base_channels // 2, 1, kernel_size=1, padding=0, stride=1, bias=True
            ),
        )
    ...
    def forward_transformer(
        ...
        for i in range(num):
            ...
            for j in range(num + pre_num):
                if i != j:
                    ...
                    warped_key_ = warp_volume(
                        keys[j],
                        depth_values_lowres.view(B, 1, D, H * W),
                        rel_pose,
                        cam_intr,
                        self.pixel_grid,
                        depth_min,
            ...
            fused_cost = self.epipolar_transformer(
                target_key=keys[i],
                warped_keys=warped_keys,
                target_value=values[i],
                warped_values=warped_values,
            )
            ...
            outputs[("depth", i, 2)], outputs[("fused_prob", i)] = depthlayer(
                fused_logits, depth_values_highres
            )
        ...
        # scale 1
        x = self.upconv_1_0(
            torch.cat([semantic_vs, self.relu(all_fused_logits)], dim=1)
        )
        x = [upsample(x)]
        if self.use_skips:
        ...
        pred_depth_s1 = F.interpolate(
            self.depth_max * self.sigmoid(self.dispconv_1(x)), scale_factor=2
        )
