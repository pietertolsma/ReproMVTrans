...
class EpipolarTransformer(nn.Module):
    ...
    def __init__(self, input_channel, output_channel, kernel_size):
        ...
        self.gate_conv = nn.Conv3d(
            gru_input_channel, output_channel * 2, kernel_size, padding=1
        )
        ...
        self.output_conv = nn.Conv3d(
            gru_input_channel, output_channel, kernel_size, padding=1
        )
    ...
    def forward(self, target_key, target_value, warped_values=None, warped_keys=None):
        ...
        if warped_values is not None:
            ...
            for key in warped_keys:
                correlation = torch.sum(
                    target_key * key, dim=1, keepdim=True
                )  # [B,1,D,H,W]
            ...
            h = torch.mean(
                values * attention_maps.repeat(1, C, 1, 1, 1, 1), dim=-1, keepdim=False
            )
        else:
            h = None
        ...
        if h is None:
            h = torch.zeros(
                (B, HC, D, H, W), dtype=torch.float, device=target_value.device
            )
